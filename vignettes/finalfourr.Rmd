...
16. For this example, I will be using the 2020 season to make a prediction as to
...
`power_plot()` allows you to visualize how a specific college basketball team's 
...
---
Querying the Data
This package includes a variety of functions designed to help users quickly 
access and explore specific subsets of the dataset. These query functions allow 
you to retrieve team-specific information, filter data by season or region, and 
examine key performance metrics across different dimensions. Whether you're 
interested in tracking a team's performance over time, comparing power ratings,
or isolating records from a specific year, these functions provide a flexible 
and user-friendly interface for working with the data. Below is a list of all 
currently available query functions:

filter_team() – Retrieve all rows for a specific team across seasons.

get_team_record() - Retrieve all of a teams wins and losses across any season 
range. 

plot_power() – Visualize a team's power rating (barthag) across seasons.

These tools are intended to streamline analysis and make it easy to derive 
insights from the underlying basketball data.
